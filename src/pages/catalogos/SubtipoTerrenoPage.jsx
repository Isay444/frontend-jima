...
const SubtipoTerrenoPage = () => {
    ...
    //Obtener datos
    const { data: subtiposTerreno = [], isLoading, isError } = useQuery({
        queryKey: ['subtiposTerreno'],
        queryFn: subtipoTerrenoApi.getAll,
    });

    //Mutacion para eliminar
    const deleteMutation = useMutation({
        mutationFn: subtipoTerrenoApi.remove,
        onSuccess: () => {
    ...
    const dataFiltrada = useMemo(() => {
        if (!globalFilter) return subtiposTerreno;
        const busqueda = globalFilter.toLowerCase();
        return subtiposTerreno.filter(terreno => 
            terreno.nombre?.toLowerCase().includes(busqueda)
        );
    }, [subtiposTerreno, globalFilter]);
    ...
    const columns = useMemo(
        () => [
            ...
            },           
            {
                ...
                header: 'Acciones',
                cell: ({ row }) => {
                    const id = row.original.idSubtipoTerreno;
                    return (
                        <div className='flex items-center gap-2'>
                            {tienePermiso('SUBTIPOTERRENO_UPDATE') && (
                                <button 
                                    onClick={() => navigate(`/catalogos/subtipos-terreno/editar/${id}`)}
                            ...
                            {tienePermiso('SUBTIPOTERRENO_DELETE') && (
                                <button 
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        titulo="Terrenos"
        descripcion="Catalógo de los terrenos para los trabajos"
        onNuevo={tienePermiso('SUBTIPOTERRENO_CREATE') ? () => navigate('/catalogos/subtipos-terreno/nuevo') : null}
        botonTexto="Terrenos"
      />
...
export default SubtipoTerrenoPage;
