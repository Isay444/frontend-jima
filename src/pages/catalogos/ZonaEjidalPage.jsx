...
const ZonaEjidalPage = () => {
    ...
    //Obtener datos
    const { data: zonaEjidal = [], isLoading, isError } = useQuery({
        queryKey: ['zonaEjidal'],
        queryFn: zonaEjidalApi.getAll,
    });

    //Mutacion para eliminar
    const deleteMutation = useMutation({
        mutationFn: zonaEjidalApi.remove,
        onSuccess: () => {
    ...
    const dataFiltrada = useMemo(() => {
        if (!globalFilter) return zonaEjidal;
        const busqueda = globalFilter.toLowerCase();
        return zonaEjidal.filter(zonaEjidal => 
            zonaEjidal.nombre?.toLowerCase().includes(busqueda)
        );
    }, [zonaEjidal, globalFilter]);
    ...
    const columns = useMemo(
        () => [
            ...
            },            
            {
                ...
                header: 'Acciones',
                cell: ({ row }) => {
                    const id = row.original.idZonaEjidal;
                    return (
                        <div className='flex items-center gap-2'>
                            {tienePermiso('ZONAEJIDAL_UPDATE') && (
                                <button 
                                    onClick={() => navigate(`/catalogos/localidades/editar/${id}`)}
                            ...
                            {tienePermiso('ZONAEJIDAL_DELETE') && (
                                <button 
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        titulo="Localidades"
        descripcion="Gestión de localidades de Hidalgo o nuevas"
        onNuevo={tienePermiso('ZONAEJIDAL_CREATE') ? () => navigate('/catalogos/localidades/nuevo') : null}
        botonTexto="Localidad"
      />
      ...
      <ConfirmDialog
        ...
        mensaje="Esta acción eliminará la localidad permanentemente. ¿Estás seguro?"
