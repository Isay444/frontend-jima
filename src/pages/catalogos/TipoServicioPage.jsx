...
const TipoServicioPage = () => {
    ...
    //Obtener datos
    const { data: tiposServicios = [], isLoading, isError } = useQuery({
        queryKey: ['tiposServicios'],
        queryFn: tipoServicioApi.getAll,
    });

    //Mutacion para eliminar
    const deleteMutation = useMutation({
        mutationFn: tipoServicioApi.remove,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['tiposServicios'] });
            setDeleteId(null);
        },
        onError: (error) => {
    ...
    const dataFiltrada = useMemo(() => {
        if (!globalFilter) return tiposServicios;
        const busqueda = globalFilter.toLowerCase();
        return tiposServicios.filter(tiposServicios => 
            tiposServicios.nombre?.toLowerCase().includes(busqueda)
        );
    }, [tiposServicios, globalFilter]);
    ...
    const columns = useMemo(
        () => [
            ...
            },            
            {
                ...
                header: 'Acciones',
                cell: ({ row }) => {
                    const id = row.original.idTipoServicio;
                    return (
                        <div className='flex items-center gap-2'>
                            {tienePermiso('TIPOSERVICIO_UPDATE') && (
                                <button 
                                    onClick={() => navigate(`/catalogos/tipos-servicio/editar/${id}`)}
                            ...
                            {tienePermiso('TIPOSERVICIO_DELETE') && (
                                <button 
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        titulo="Áreas"
        descripcion="Gestión de áreas o departamentos de la empresa"
        onNuevo={tienePermiso('TIPOSERVICIO_CREATE') ? () => navigate('/catalogos/tipos-servicio/nuevo') : null}
        botonTexto="Área"
      />
      ...
      <ConfirmDialog
        ...
        titulo="¿Eliminar el tipo de área?"
        mensaje="Esta acción eliminará el tipo de área permanentemente. ¿Estás seguro?"
