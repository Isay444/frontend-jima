...
const MunicipioFormPage = () => {
  ...
  const { data: municipioData, isLoading } = useQuery({
    queryKey: ['municipio', id],
    queryFn: () => municipioApi.getById(id),
    enabled: esEdicion,
  });

  useEffect(() => {
    if (esEdicion && municipioData) {
      // Mapear si el backend devuelve idArea (plural) o idArea (singular)
      const { idMunicipios, ...rest } = municipioData;
      reset(rest);
    }
  }, [municipioData, esEdicion, reset]);

  const mutation = useMutation({
    mutationFn: (data) => esEdicion ? municipioApi.update(id, data) : municipioApi.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['municipios'] });
      navigate('/catalogos/municipios');
    },
    onError: (error) => {
      setErrorServidor(error.response?.data?.message || 'Error al guardar el municipio');
    },
  });
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-slate-900">
          {esEdicion ? 'Editar Municipio' : 'Nuevo Municipio'}
        </h2>
        <p className="text-sm text-slate-500 mt-1">
          Complete los campos para {esEdicion ? 'actualizar' : 'crear'} el municipio.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
          ...
          >
            {mutation.isPending ? 'Guardando...' : 'Guardar Municipio'}
          </button>
        </div>
      </form>
    </div>
...
export default MunicipioFormPage;