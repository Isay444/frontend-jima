...
import * as municipio from '@/api/municipioApi';
...
const MunicipioPage = () => {
    ...
    const { data: municipios = [], isLoading, isError } = useQuery({
        queryKey: ['municipios'],
        queryFn: municipio.getAll,
    });

    //Mutacion para eliminar
    const deleteMutation = useMutation({
        mutationFn: municipio.remove,
        onSuccess: () => {
    ...
    const dataFiltrada = useMemo(() => {
        if (!globalFilter) return municipios;
        const busqueda = globalFilter.toLowerCase();
        return municipios.filter(municipios => 
            municipios.nombre?.toLowerCase().includes(busqueda)
        );
    }, [municipios, globalFilter]);
    ...
    const columns = useMemo(
        () => [
            ...
            },            
            {
                ...
                header: 'Acciones',
                cell: ({ row }) => {
                    const id = row.original.idMunicipios;
                    return (
                        <div className='flex items-center gap-2'>
                            {tienePermiso('MUNICIPIO_UPDATE') && (
                                <button 
                                    onClick={() => navigate(`/catalogos/municipios/editar/${id}`)}
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        titulo="Municipios"
        descripcion="Gestión de municipios del estado de Hidalgo o nuevos"
        onNuevo={tienePermiso('MUNICIPIO_CREATE') ? () => navigate('/catalogos/municipios/nuevo') : null}
        botonTexto="Municpio"
      />
      ...
      <ConfirmDialog
        ...
        mensaje="Esta acción eliminará el municipio permanentemente. ¿Estás seguro?"
