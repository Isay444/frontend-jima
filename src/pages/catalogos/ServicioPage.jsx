...
const ServicioPage = () => {
    ...
    const { data: servicios = [], isLoading, isError } = useQuery({
        queryKey: ['servicios'],
        queryFn: servicioApi.getAll,
    });

    //Mutacion para eliminar
    const deleteMutation = useMutation({
        mutationFn: servicioApi.remove,
        onSuccess: () => {
    ...
    const columns = useMemo(
        () => [
            ...
            }, 
            {
                ...
                header: 'Descripción',
                cell: info => <span className='font-medium whitespace-normal w-full'>{info.getValue()}</span>,
            },
            {
              accessorKey: 'nombreTipoServicio',
              header: 'Tipo de servicio',
              cell: info => {
                const tipoServicio = info.getValue();
                return tipoServicio ? <span>{tipoServicio}</span> : <span className="text-slate-400 block max-w-xs truncate">—</span>;
              } 
            },
            {
                id: 'acciones',
                header: 'Acciones',
                cell: ({ row }) => {
                    const id = row.original.idServicio;
                    return (
                        <div className='flex items-center gap-2'>
                            {tienePermiso('SERVICIO_UPDATE') && (
                                <button 
                                    onClick={() => navigate(`/catalogos/servicios/editar/${id}`)}
    ...
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-md">
        <p className="text-sm text-red-600 text-center">Error al cargar los servicios</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        titulo="Servicios"
        descripcion="Catalógo de los servicios ofrecidos por la empresa"
        onNuevo={tienePermiso('SERVICIO_CREATE') ? () => navigate('/catalogos/servicios/nuevo') : null}
        botonTexto="Servicios"
      />
      ...
      <ConfirmDialog
        open={!!deleteId}//{Boolean(deleteId)}
        titulo="¿Eliminar el servicio?"
        mensaje="Esta acción eliminará el servicio permanentemente. ¿Estás seguro?"
