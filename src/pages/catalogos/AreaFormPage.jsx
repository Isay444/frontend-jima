...
const AreaFormPage = () => {
  ...
  useEffect(() => {
    if (esEdicion && areaData) {
      // Mapear si el backend devuelve idArea (plural) o idArea (singular)
      const { idArea, ...rest } = areaData;
      reset(rest);
    }
  }, [areaData, esEdicion, reset]);

  const mutation = useMutation({
    mutationFn: (data) => esEdicion ? areaApi.update(id, data) : areaApi.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['areas'] });
      navigate('/catalogos/areas');
    },
    onError: (error) => {
      setErrorServidor(error.response?.data?.message || 'Error al guardar el área');
    },
  });
    ...
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Complete los campos para {esEdicion ? 'actualizar' : 'crear'} el área.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
          ...
          >
            {mutation.isPending ? 'Guardando...' : 'Guardar Área'}
          </button>
        </div>
      </form>
    </div>
