...
const IngenieroPage = () => {
    ...
    //Obtener datos
    const { data: ingeniero = [], isLoading, isError } = useQuery({
        queryKey: ['ingeniero'],
        queryFn: ingenieroApi.getAll,
    });

    //Mutacion para eliminar
    const deleteMutation = useMutation({
        mutationFn: ingenieroApi.remove,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['ingeniero'] });
            setDeleteId(null);
        },
        onError: (error) => {
            alert('Error al eliminar el ingeniero');
            console.error('Error al eliminar el ingeniero:', error);
        }
    });

    //Filtrado local por nombre
    const dataFiltrada = useMemo(() => {
        if (!globalFilter) return ingeniero;
        const busqueda = globalFilter.toLowerCase();
        return ingeniero.filter(ingeniero => 
            ingeniero.nombre?.toLowerCase().includes(busqueda)
        );
    }, [ingeniero, globalFilter]);
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'especialidad', // del dto response del backend
                header: 'Especialidad',
                cell: info => <span className='font-medium'>{info.getValue()}</span>,
            },
            {
            ...
            },
            {
                ...
                header: 'Acciones',
                cell: ({ row }) => {
                    const id = row.original.idIngeniero;
                    return (
                        <div className='flex items-center gap-2'>
                            {tienePermiso('INGENIERO_UPDATE') && (
                                <button 
                                    onClick={() => navigate(`/ingenieros/editar/${id}`)}
    ...
    if(isLoading){
        return(
            <div className='flex justify-center items-center min-h-[400px]'>
                <p className="text-slate-500">Cargando ingenieros...</p>
            </div>
        );
    }
    ...
    return (
      <div className="p-4 bg-red-50 border border-red-200 rounded-md">
        <p className="text-sm text-red-600 text-center">Error al cargar ingenieros</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        titulo="Ingenieros"
        descripcion="Gestión de ingenieros de Hidalgo o nuevos"
        onNuevo={tienePermiso('INGENIERO_CREATE') ? () => navigate('/ingenieros/nuevo') : null}
        botonTexto="Ingeniero"
      />
      ...
      <ConfirmDialog
        ...
        titulo="¿Eliminar ingeniero?"
        mensaje="Esta acción eliminará el ingeniero permanentemente. ¿Estás seguro?"
